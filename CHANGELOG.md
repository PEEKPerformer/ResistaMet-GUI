...
Development branch `phase0/reviewable-baseline`. The measuring code is separated from the PySide6 window into a typed settings schema, a headless run layer and a localhost API; a second front end (a Tauri desktop app) and scripts drive the same runs and write the same files. The PySide6 window remains the released application. New pages: [`docs/api.md`](docs/api.md), [`docs/desktop.md`](docs/desktop.md), [`docs/gpib.md`](docs/gpib.md); each says what has been checked on hardware and what only against the simulator.

**If you parse the data files, read "Changed → Data files" first.**

### Added
- **Headless run layer and settings schema.** `resistamet_gui/session` holds the measurement procedures with no Qt in them (`ContinuousRun`, `VdpRun`, `MeasurementSession`); the PySide6 worker threads are thin adapters over them. `resistamet_gui/schema` gives every setting a typed model with its default and bounds, and one resolver turns a profile plus a client's values into run settings, so both front ends start identical runs. JSON Schemas are exported to `contracts/`.
- **Localhost HTTP + WebSocket API** (`python -m resistamet_gui.api`, optional extra `pip install -e ".[api]"`): start / stop / pause / mark, prompts for the decisions only a person can make (touch-safety acknowledgement, van der Pauw rewiring), a resumable event stream, profile and settings routes, results and four-point map routes, a one-line JSON handshake, a bearer token, and an ordered shutdown on stdin close, `POST /session/shutdown`, SIGTERM or SIGINT (output off, file finalized, then exit). Checked against a Keithley 2420 with a frozen Windows build on 2026-09-18; everything else against the simulator. Unversioned and subject to change.
- **Tauri desktop app** (`desktop/`), in development: all six modes, results browser, profile settings, blocking prompts, a four-point spot table and map, a question before closing the window stops a run. Installers are built for `desktop-v*` tags; none is released yet.
- **One process per instrument.** Every ResistaMet process takes an operating-system lock per instrument address (`~/.resistamet/locks/`) for the length of a run, a connection test or a cable null, so two windows, or a window and a script, can no longer interleave SCPI on one bus. A refused start says `… is in use by another ResistaMet process`.
- **VISA backend choice per machine** (`visa_library`: Automatic, vendor VISA, pyvisa-py) in both Settings dialogs. Needed on a Mac, where an installed NI-VISA has no GPIB driver behind it.
- **Built-in user-space driver for NI GPIB-USB adapters** on macOS and Linux (`resistamet_gui/gpib_usb`, MIT, through pyvisa-py; needs `pip install -e ".[usb]"`, libusb and Python 3.10+). Identify, resistance runs, stop, restart and shutdown ran on a GPIB-USB-HS with a Keithley 2400 on 2026-09-18. Framed reads are capped at 1024 bytes per instruction (a longer framed answer left one adapter unusable until replugged), and 35 kB answers, sweeps and a 45-minute run followed on 2026-09-21. A timeout never expires before the time asked for and bounds a read as a whole, as NI's driver does; an unplugged adapter ends the operation at once with `VI_ERROR_CONN_LOST` and is found again after a replug. The adapter instructions NI's own driver uses for large transfers and serial poll were added from USB captures, partly run on 2026-09-21, now send NI's exact messages, and are off unless `NI_GPIB_USB_INSTRUCTIONS=1`. Only the GPIB-USB-HS is supported for them; other adapter models and Linux are untested. The package has its own README.
- **`gpib_interface` setting** for Prologix / AR488 adapters (pyvisa-py only). The interface opens and scans report it; **a run does not connect through it yet**, because connecting requires the address to appear in the resource list and pyvisa-py lists nothing behind such an adapter. Tested against a scripted stand-in only.
- **`--check-visa [bus]`** on the backend: one JSON line saying which VISA implementation loads, its version, whether libusb is available, which NI adapters are on USB, and optionally the resource list. The diagnostic for "no instruments detected".
- **Four-point spots and maps.** A four-point run can carry a spot (map id, index, label, optional position and array angle). A map is the set of runs sharing a map id, assembled from the run files, served at `GET /maps/{map_id}` and written beside the runs as `<map_id>_map.json`. The PySide6 window sends a spot with every four-point run, minting a map per user and sample name. New settings `fpp_sample_shape`, `fpp_sample_diameter_mm`, `fpp_sample_width_mm`, `fpp_sample_length_mm`, `fpp_array_angle_deg`, `fpp_edge_warn_pct`, `fpp_position_correction` describe the sample outline: a spot with a probe tip off the sample is refused before the output turns on, and one near an edge is warned about with the size of the effect. The position effect is reported, never applied to the rows.
- **Warning when a four-point V/I is negative** (open or swapped sense leads), once per run.

### Changed
- **Data files: names.** The source value is kept in the file name. Up to 1.12.3 everything after the last dot was replaced by the extension, so `…_4PP_1.00mA` was written as `…_4PP_1.csv`; it is now `…_4PP_1.00mA.csv` (likewise `…_VSRC_0.500V.csv`, `…_DATA_sweep_0.0to1.0.csv`). Scripts that match the old names need updating.
- **Data files: never overwritten.** Two runs of one sample within the same second used to share a name, and the second replaced the first. The second now gets `-2` (then `-3`, …) appended to the base name, and files are created in exclusive mode.
- **Data files: new header keys.** `effective.voltage_compliance_V` (resistance mode, manual range: the voltage limit the instrument reports after configuration) or `effective.voltage_compliance_V_at_configure` (Auto range: the same read-back, which auto-ohms does not hold to); `client.name` / `client.version` for runs started through the API; the `spot.*` block for four-point runs that carry a spot. All appear between `params.*` and `units:`. A parser that reads `# key: value` lines generically is unaffected; one that expects a fixed number of header lines is not.
- **Data files: new footer keys.** Every four-point file now ends with `spot_stats.*` (n, excluded rows, end reason, and mean / sd / RSD / `u_stat` / `u_inst` / `u_total` for Rs, ρ and σ) after `duration_s`, about 25 more trailing lines. `parse_metadata` reads them; it also gained `text_keys=` to return identifiers as text.
- **Data files: a stop during the settling wait finalizes the file** with its footer; it used to be left without one.
- **Machine-local settings** (`gpib_address`, `visa_library`, `gpib_interface`) are stored in `~/.resistamet/machine.json` instead of `config.json` under `machines.<hostname>`, which stopped matching whenever macOS changed the hostname. An existing entry is carried over on first start.
- **`config.json` saves** are atomic and merge under a file lock, so two programs on one PC no longer undo each other's edits; an unreadable file is kept and copied aside instead of being replaced by defaults.
- **I-V sweep compliance is bounded in its own unit**: up to 3.15 A for a voltage-sourced sweep, up to 210 V for a current-sourced one (it was 3 for both). The touch-safety check covers a current-sourced sweep through its voltage compliance.
- **Event marks queue.** Two marks before the next sample are both written, joined with `; `; the second used to replace the first.
- **Stop is immediate during a settling delay**, and a timed run's duration counts measuring time, so a run paused past its deadline no longer ends on resume.
- **Log messages use the mode names on the tabs** ("Four-point probe", not `four_point`).

### Fixed
- **Compliance was never flagged in Resistance mode.** The 2400 and 2420 do not set the status word's compliance bit in the ohms function (bench, 2026-09-18), so rows measured in compliance were written as `OK` with a plausible, wrong resistance. In manual range a row is now `V_COMP` when the measured voltage reaches 99 % of the limit the instrument reports; in Auto range, where the instrument moves its own limit, on the status bit or the overflow value.
- **Output format and compression settings never reached a run.** They were saved and ignored; every run wrote uncompressed CSV. They now apply. Because a long-forgotten HDF5 or compression choice would otherwise change the next run's file without warning, stored Output choices are reset to the CSV defaults once on upgrade.
- **Touch-safety "Don't show again" did not persist** in the PySide6 window (it called a method that does not exist, and the error was swallowed).
- **The PySide6 window never wrote its log file.** `~/.resistamet/logs/` was provided for and nothing turned it on, so only warnings reached the terminal. The window now writes `resistamet_<date>.log` there (5 MB, three rotations) and logs to the terminal as before; a home directory that cannot take the file leaves terminal logging in place with a warning.
