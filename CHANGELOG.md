...
- **Built-in user-space driver for NI GPIB-USB adapters** on macOS and Linux (`resistamet_gui/gpib_usb`, MIT, through pyvisa-py; needs `pip install -e ".[usb]"`, libusb and Python 3.10+). Identify, resistance runs, stop, restart and shutdown ran on a GPIB-USB-HS with a Keithley 2400 on 2026-09-18. The adapter instructions NI's own driver uses for large transfers and serial poll were added later from USB captures, have not run on hardware, and are off unless asked for. Other adapter models and Linux are untested.
