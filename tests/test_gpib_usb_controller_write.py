...
class TestWrite:
    ...
    def test_a_split_write_is_bounded_by_its_timeout_as_a_whole(self):
        # Like a read (§7.1, §10.10.2): the deadline is 0xfd's shortest expiry, 16.78 s; the
        # second chunk gets the code for the time left, and none starts once it has passed.
        script = [
            ('out', p.register_read_message(t.USB_B_SERIAL_REGISTERS)),
            ('in', regread_reply([0x78, 0x56, 0x34, 0x12]), 32),
        ] + attach_script()[2:] + address_listener(code=0xFD) + [
            ('out', p.write_message(bytes(0xFFFF), 0xFD, send_eoi=False)), ('in', status_reply(0x0D), None, 15.0),
            ('out', p.write_message(bytes(0xFFFF), 0xFC, send_eoi=False)), ('in', status_reply(0x0D), None, 2.0),
        ]
        transport = ScriptedTransport(script)
