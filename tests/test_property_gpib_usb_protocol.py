...
The seeded checks at the end draw with ``random.Random`` under a fixed seed
instead, thousands of cases per test, so a failure reproduces and its message
carries the offending bytes: every parser fed anything, counts and buffer
sizes for every length, block sequences split and cut, framed read replies
built at random, the ``m e`` table and the timeout table's order.
