        ...
        try:
            ...
            try:
                if self.mode == 'resistance':
                    configured = self._configure_resistance(measurement_settings, nplc)
                elif self.mode == 'source_v':
                    configured = self._configure_source_v(measurement_settings, nplc)
                elif self.mode == 'source_i':
                    configured = self._configure_source_i(measurement_settings, nplc)
                elif self.mode == 'four_point':
                    configured = self._configure_four_point(measurement_settings, nplc)
                    if configured is None:
                        return  # pre-flight refused the power envelope
                elif self.mode == 'sweep':
                    configured = self._configure_sweep(measurement_settings, nplc)
                else:
                    configured = None
