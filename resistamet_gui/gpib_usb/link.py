...
§10.6.5). ``Controller`` inherits these from ``_ExchangeMixin``, which has
no state of its own: it works on the controller's ``_transport``,
``_model``, ``_raw``, ``_host_stopped``, ``_drained``, ``_resync_pending``
and ``_infinite_wait_s``. The constants are re-exported by ``controller``.
"""
import logging
...
class _ExchangeMixin:
    """The exchange primitives and fault recovery of ``Controller`` (see the module docstring)."""
