...
whole sequence so nothing interleaves with it. The adapter also has an
instruction for the serial poll (§10.5.4), a ``Controller`` primitive;
``serial_poll`` here sends it only on a controller built for NI's
instructions.
"""
import logging
...
def serial_poll(controller: Controller, pad: int, sad: Optional[int] = None,
    ...
    """The status byte of one device.

    §5.9: SPE sequence, standby, one-byte read, SPD UNT. This is the poll
    that ran on the bench. A controller built with ``ni_instructions``
    sends the adapter's 0x10 instruction instead (§10.5.4), which has not.
    """
    if controller.ni_instructions:
        return controller.serial_poll_instruction(pad, sad, timeout_s)
