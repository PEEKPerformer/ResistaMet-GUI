...
Writes ``contracts/settings.schema.json``, ``contracts/events.schema.json``,
``contracts/maps.schema.json`` and ``contracts/session.schema.json``.
...
def main():
    ...
    for name, schema in (('settings', settings_schema()), ('events', events_schema()),
                         ('maps', maps_schema()), ('session', session_schema())):
