...
| Timeout expiries, long answers read in framed pieces of 1024 bytes | The same unit, 2026-09-21; the short codes' expiries again on 2026-09-23 (spec §10.11) |
| The instructions NI's own driver uses: *raw* reads and writes on the adapter's second endpoint pair, and serial poll as a single instruction | Written on 2026-09-19 from USB captures of NI's driver on Windows, driving a second GPIB-USB-HS (013CC9DF) with a Keithley 2420. An earlier form was tried on 01CEE482 on 2026-09-21 (spec §11.2). In their present form, NI's own messages byte for byte, they ran on 01CEE482 on 2026-09-23 (spec §10.11): a raw read with nothing to read ended at its timeout code's expiry, answers of 1536 and 35 000 bytes arrived whole, raw writes of 2049, 3000 and 6000 bytes went through, and a write to an empty address failed at once with no listeners and the next query answered. The single-instruction serial poll's status byte agreed with `*STB?` (2026-09-21). Off by default. |
| The `GPIB0::INTFC` board resource | Run on 01CEE482, 2026-09-23 (spec §10.11): IFC, REN on and off, command bytes, with an instrument session on the same board answering throughout. No run of its data transfers is recorded. ResistaMet's own measurements do not use it. |
| Service-request wait | The write that arms the adapter for a service request, and the request it then reports, ran on 01CEE482, 2026-09-23 (spec §10.11). The wait as written now ran on the same unit on macOS the same day: five service requests in one session, each seen in 7-8 ms with status byte 96; waits with nothing pending timed out at 1.14-1.16 s for 1 s; other calls from another thread went through during a wait, and a close ended one in 0.5 s. The second unit, 013CC9DF with a Keithley 2420, reported the request differently the same day (spec §10.12). 01CEE482 pushed 8 bytes carrying the instrument's status byte, which the adapter had read by polling the instrument itself. 013CC9DF sent a 4-byte packet, `31 a5 nn 00`, with no status byte, and did not poll the instrument. For that form the wait returns None at once, and the caller has to serial-poll the instrument for its status byte and clear the cause of the request (on the 2420, `*ESR?`). The wait raised a protocol error on the 4-byte packet on the bench. The version that accepts it then ran on that unit on macOS the same day: five service requests in one session, each returned None in 2-3 ms and each cleared by `*ESR?`. In a separate session a 1 s wait before `*OPC` timed out at 1.18 s, a wait after `*OPC` returned None in 2 ms, the serial poll that followed returned 96 and, after `*ESR?`, 0, and a wait with nothing pending timed out at 1.18 s. Nothing in ResistaMet or pyvisa-py 0.8.1 calls it. |
| Unplugging the adapter mid-run and plugging it back in | 01CEE482, 2026-09-21 and 2026-09-23. How the operating system reports it differs by platform (below). The driver's check for the macOS case ran on the bench on 2026-09-23 with a single look and was too early: libusb still listed the adapter for about 10 ms, so the first call reported an I/O error. The check now waits up to 100 ms, and in that form ran on the same unit on macOS the same day: with the cable pulled during a loop of `*IDN?` queries, the first failed call returned `VI_ERROR_CONN_LOST` after 0.116 s with no recovery attempted, and the next failed at once the same way. After the replug the adapter was back in 4.7 s and a new session in the same process answered in 0.138 s. |
...
| Linux | Run on 2026-09-23 in an Ubuntu 26.04 arm64 virtual machine on the Mac, the adapter passed through (spec §10.11); not on a Linux machine of its own. The udev rule in the [package README](../resistamet_gui/gpib_usb/README.md#linux) was checked there, and so was the detach of a kernel driver that had claimed the adapter. Without the rule, opening the adapter fails with a message that names the device node and the rule. |

The default matters because of how reads are sized. Which read instruction goes out depends on how many bytes the caller *asks* for, not on how many arrive, and PyVISA asks for 20 480 bytes on every read. With NI's instructions switched on, every reply, a single `:READ?` included, would come in through the raw path, which in its present form has run on one adapter, on one day. So they stay off by default, and every transfer uses the framed instructions that have the most bench time, in pieces of at most 1024 bytes.
...
- **Timeouts are coarse.** The adapter takes a code from a fixed table, not a time. A timeout goes out as the code NI's driver sends for it, the smallest nominal limit not below it (5 s is the 10 s code, 0xfd; a sweep's 11 to 16 s the 30 s code, 0xfe), except that 264 to 300 ms go out as 0xfb and 268 to 300 s as 0x02, because the next code down ends sooner than asked. What the adapter then waits differs by unit: for 0xfd, 16.78 s on the unit NI's driver was captured on and 20.0 s on the bench unit. A read is bounded as a whole by the longer of the two, not by the timeout set, so with the instrument silent **a 5 s timeout reports after about 20 s**, with whatever bytes had arrived. `VI_TMO_INFINITE` is stopped by the driver after 600 s. The per-code figures are in [Troubleshooting](troubleshooting.md#a-timeout-takes-about-20-s-although-5-s-was-asked) and the [package README](../resistamet_gui/gpib_usb/README.md#what-a-timeout-means).
- **An unplugged adapter.** The operation in flight, and every later one on that session, fails at once with `VI_ERROR_CONN_LOST`; the driver does not retry on a device that is gone. On Linux libusb reports the device gone at the first failed call. On macOS it does not: the transfer in flight fails with an I/O error and every later request with "Other error" (spec §10.11), so after such an error the driver watches the USB bus for up to 100 ms, since libusb keeps an unplugged adapter listed for about 10 ms, and reports it gone if it leaves. An error with the adapter still there is reported up to 100 ms later for that. After the adapter is plugged back in, the next connection finds it again with no restart. When the cable was pulled mid-run on the bench (2026-09-21), the run's file was finalised, but the output-off at cleanup could not reach the instrument, so its **output stayed on** until the next session addressed it.
