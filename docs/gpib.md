...
| Unplugging the adapter mid-run and plugging it back in | 01CEE482, 2026-09-21 and 2026-09-23. How the operating system reports it differs by platform (below). The driver's check for the macOS case was written after that run and has not met hardware. |
...
- **An unplugged adapter.** The operation in flight, and every later one on that session, fails at once with `VI_ERROR_CONN_LOST`; the driver does not retry on a device that is gone. On Linux libusb reports the device gone at the first failed call. On macOS it does not: the transfer in flight fails with an I/O error and every later request with "Other error" (spec §10.11), so after such an error the driver looks for the adapter on the USB bus, and reports it gone if it is not there. After the adapter is plugged back in, the next connection finds it again with no restart. When the cable was pulled mid-run on the bench (2026-09-21), the run's file was finalised, but the output-off at cleanup could not reach the instrument, so its **output stayed on** until the next session addressed it.
