...
| Service-request wait | The write that arms the adapter for a service request, and the request it then reports, ran on 01CEE482, 2026-09-23 (spec §10.11). The wait as written now has not run as a whole, and nothing in ResistaMet or pyvisa-py 0.8.1 calls it. |
| Unplugging the adapter mid-run and plugging it back in | 01CEE482, 2026-09-21 and 2026-09-23. How the operating system reports it differs by platform (below). The driver's check for the macOS case ran on the bench on 2026-09-23 with a single look and was too early: libusb still listed the adapter for about 10 ms, so the first call reported an I/O error. The check now waits up to 100 ms; in that form it has not met hardware. |
